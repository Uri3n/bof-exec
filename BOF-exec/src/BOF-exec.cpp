...
uint32_t
object_virtual_size(object_context* ctx) {
...
bool
object_execute(object_context* ctx, const char* entry, char* args, const uint32_t argc) {
...
void*
resolve_object_symbol(const char* symbol) {
...
void
object_relocation(const uint32_t type, void* needs_relocating, void* section_base) {
...
bool
process_object_sections(object_context* ctx) {
...
|_.__/ \___/|_|        \___/_/\_\___|\___|
    ...
    if(argc < 2) {
        ...
        std::cout << R"(            BOF-exec bof.o)"                              << std::endl << std::endl;
    ...
    if(!std::filesystem::exists(argv[1]) ||
        (std::filesystem::path(argv[1]).extension().string() != ".o" && //only permit .o or .obj
